...
class WebsiteTransportService(http.Controller):
    ...
    def _rate_limit_ok(self):
        ...
        key = "att_transport_orders.website_quote.%s" % ip
    ...
    def _get_next_salesperson(self, team):
        ...
        key = "att_transport_orders.website_quote.last_user_id"
        icp = request.env["ir.config_parameter"].sudo()
    ...
    def _format_body(self, mode, post, order=False):
        return Markup("""
    ...
    def service_list(self, **kw):
        modes = request.env["product.product"].sudo().search([
            ("active", "=", True),
            ("website_published", "=", True),
        ], order="name")

        return request.render("att_transport_orders.website_service_list", {
            "modes": modes,
        })
    ...
    @http.route(
        "/dich-vu/<model('product.product'):mode>/bao-gia",
        type="http",
        auth="public",
        website=True,
        sitemap=False,
    ...
    def quote_form(self, mode, **kw):
        ...
        return request.render("att_transport_orders.website_quote_form", {
            "mode": mode,
            "errors": [],
            "values": {},
        })
    ...
    @http.route(
        "/dich-vu/<model('product.product'):mode>/bao-gia/submit",
        type="http",
        auth="public",
        website=True,
        methods=["POST"],
    ...
    def quote_submit(self, mode, **post):
        ...
        # mode CHÍNH LÀ sản phẩm dịch vụ (hình thức vận chuyển) — dùng thẳng
        # làm product_id của dòng, không tạo sản phẩm tạm nữa.
        order_line_vals = {
            "product_id": mode.id,
            "name": mode.name,
            "product_uom_qty": 1.0,
        ...
        body = self._format_body(mode, post, order=order)
    ...
    def quote_thank_you(self, so=None, **kw):
        ...
        return request.render("att_transport_orders.website_quote_thank_you", {
            "order": order,
        })
