...
class AttTransportOrder(models.Model):
    ...
    # Mã lệnh = số thứ tự (sequence_no, cấp 1 lần lúc tạo) + biển số xe ở
    # cuối — biển số có thể đổi trước lúc xác nhận nên name tính lại mỗi khi
    # vehicle_id đổi, không gán cứng lúc tạo như sequence_no.
    sequence_no = fields.Char('Số thứ tự', default='/', copy=False, readonly=True)
    name = fields.Char('Mã lệnh vận chuyển', compute='_compute_name', store=True, copy=False)
    ...
    cost_line_ids = fields.One2many('att.transport.cost.line', 'transport_order_id',
                                    'Dòng chi phí phát sinh', copy=False)
    ...
    def _check_duplicate_sale_order_line(self):
        for rec in self:
            ...
            if duplicated:
                raise UserError(_('Dòng SO %s đã có lệnh vận chuyển khác chưa hủy.')
                                % rec.sale_order_line_id.display_name)

    @api.depends('sequence_no', 'vehicle_id.license_plate')
    def _compute_name(self):
        for rec in self:
            base = rec.sequence_no or '/'
            plate = rec.vehicle_id.license_plate
            rec.name = f'{base}-{plate}' if plate else base
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('sequence_no', '/') in ('/', False):
                vals['sequence_no'] = self.env['ir.sequence'].next_by_code('att.transport.order') or '/'
    ...
    def _notify_zns(self, partner, body):
        """Gửi ZNS — nếu có cài att_zalo_connector + đối tác đã liên kết Zalo
        thì gửi tạm qua OA chat; không thì ghi chatter để không chặn vận hành."""
        self.ensure_one()
        try:
    ...
    def action_confirm_dest(self):
        for rec in self:
            ...
            address = self.env['ir.config_parameter'].sudo().get_param(
                'att.pnv.return_address', '(chưa cấu hình att.pnv.return_address)')
    ...
    def action_report_incident(self):
        ...
        managers = self.env.ref('att_transport_orders.group_th_dispatcher').users.mapped('partner_id')
        self.message_post(
    ...
    def _push_cost_lines(self):
        ...
        for line in self.cost_line_ids:
            ...
            if line.paid_by == 'customer' and line.expense_type == 'so_line':
                # Đã có dòng báo giá từ khâu báo giá (_th_sync_quote_line)
                # → khách đã thấy/đã tính tiền, chỉ đánh dấu xong, KHÔNG
                # tạo SOL trùng lần 2.
                ...
                bill = self.env['account.move'].create({
                    ...
                    'ref': _('Chi phí lệnh %s') % self.name,
        ...
        if skipped:
            self.message_post(
                body=Markup('Lưu ý: <b>%d</b> dòng chi phí chưa được xử lý do thiếu duyệt hoặc thiếu thông tin.') % skipped,
                message_type='notification', subtype_xmlid='mail.mt_note')
